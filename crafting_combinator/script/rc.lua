...
function _M:update(forced)
	...
	if self.settings.mode == 'rec' or self.settings.mode == 'use' then
		self:find_recipe()
...
function _M:find_recipe()
	...
	for i, recipe in pairs(recipes) do
		...
		if not recipe.recipe.hidden and recipe.recipe.enabled then
			...
			param.count = self.settings.differ_output and index or
				(self.settings.divide_by_output and round(count / recipe.amount) or count)
			param.index = index
			index = index + 1
...
function _M:find_ingredients_and_products()
	...
	if recipe then
		...
		table.insert(params, {
			...
			count = util.simulate_overflow(math.floor(tonumber(recipe.energy) * self.settings.time_multiplier *
				crafting_multiplier)),
...
function _M:open(player_index)
	local root = gui.entity(self.entity, {
		...
		gui.section {
			...
			gui.checkbox('output-shortage', self.settings.output_shortage, { tooltip = true }),
		}
	}):open(player_index)
